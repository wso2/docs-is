...
If **X509 authentication** is specified, the WSO2 IS will authenticate
...
5.  In order for the JVM to trust your newly created certificate import
    ...
    !!! note "Got the permission denied error?"
...
6.  Now you have created the CA to sign the certificate. To create the
    ...
    4.  Once it is done you need to sign the CSR, which requires the CA
        root key.
...
7.  Next, create a new trust store and import the server certificate
    ...
    !!! info "CN"
        The User objects in the LDAP directory hierarchy have designators
...
2.  Replace your keystore file path, keystore password, trust store file
    ...
    Certificate](#working-with-certificates)
    ...
    !!! note
        ...
        -   The `            truststoreFile           ` attributes specifies
...
1.  Open the `          certificate-validation.xml         ` file in the
...
your use case:  
-   [Disabling Certificate Validation in an Unstarted WSO2 IS Pack](#disabling-certificate-validation-in-an-unstarted-wso2-is-pack)
-   [Disabling Certificate Validation in an Already-started WSO2 IS Pack](#disabling-certificate-validation-in-an-already-started-wso2-is-pack)
-   [Using an identity claim for the X509 certificate or working with read only user stores](#using-an-identity-claim-for-the-x509-certificate-or-working-with-read-only-user-stores)                
-   [Using a wso2 claim for the X509 certificate](#Using-a-wso2-claim-for-the-x509-certificate)
...
-   Refer this
    [link](../../administer/browsing-the-h2-database)
...
app](../../develop/deploying-the-sample-app) for information on how to configure
this.
    ...
    2.  First factor
        ...
        -   When using X509 as first step authentication, you need to
            ...
            !!! note
                ...
                [here](../../learn/configuring-users#creating-a-new-user-using-the-management-console).

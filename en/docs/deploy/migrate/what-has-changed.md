...
## Webapp
This section covers the updates related to the Console application and the MyAccount application on Identity Server 6.0.0.
...
- Add the certificate to the key store and select the certificate through the UI.
...
## User management
This section contains the changes to the User Management functionalities of IS 6.0.0.
...
Updating the ```ExpiryTime``` of the ```identity.xml``` file does not affect admin initiated password reset flow in IS 6.0.0. This is because usually the code of the admin-initiated password reset flow is expected to have a longer validity period than the other flows. Therefore  If you need to update the code validity period of this add the following configuration to the ```deployment.toml``` file.
...
- The ```totpauthethentication``` endpoint, which was packed with the product is now an integral part of the authentication portal. Following are the steps to integrate their changes to Identity Server 6.0.0 for users with customized TOTP pages.
    1. Merge the customizations to the authentication portal app. The pages related to TOTP available in the authentication portal are:
    ...
    3. Unlike the earlier versions of the Identity Server, the TOTP authenticator of IS 6.0.0 does not offer to send the TOTP via email as a default option. Add the following configuration to the ```deployment.toml``` file to enable sending the TOTP through Email.
...
### Application roles in application management flow
...
In Identity Server 6.0.0, the event attribute has been renamed to events and the default value has been added to the events list. Hence, a logout token generated in IS 6.0.0 would be as follows.
``` js
{
   "iss": "https://server.example.com",
   "sub": "248289761001",
...
Make sure to update all the client applications that use OIDC Backchannel log out to work with the new format of the logout token.
...
- The OIDC scopes requested by a client application were displayed under the permissions section of the user consent page. These requested OIDC scopes are no longer mentioned under the permissions section of the user consent page. Only the relevant claims will be displayed.
...
This section contains the details of the updates related to the user management section in IS 6.0.0.

### Configuring custom user store managers
In earlier versions of WSO2 IS, custom user store managers were enabled by adding the following configuration to the ```deployment.toml``` file.
...
This configuration requires the developer to enter the names of all existing user store managers together with the new user store managers that needed to be enabled.

In Identity Server 6.0.0, a new configuration has been introduced to enable the custom user store managers but adding the name of the new user store manager only.

You may use the following configuration on ```deployment.toml``` file to configure new custom user store managers
...
## Provisioning
This section contains the details of the updates related to the provisioning section in IS 6.0.0.

### Provisioning unmapped roles in JIT provisioning
In Identity Server 5.11.0, provisioning of roles in the JIT provisioning flow took place only for roles that had role mappings configured.
...
## Scopes
This section contains the details of the updates related to the scopes section in IS 6.0.0.
...
In the previous versions of the Identity Server, JWT access token signing is done using the tenant of the user accessing the application. This behavior has been improved to maintain consistency with the ID token which is signed using the application’s tenant by default. This behavioral change will also simplify token validation in the application, especially SaaS applications.
...
If the previous behavior is required, the regex pattern validation feature can be disabled by adding the following configuration to the ```deployment.toml``` file.
...
#### Removal of duplicate entries in the SCIM2 user's response

In the older versions of the Identity Server, the responses of the SCIM2 Users endpoint did not contain duplicate entries of the same user (it is possible to have duplicate usernames in separate user stores).

In Identity Server 6.0.0, the SCIM2 User responses will contain duplicate entries of the same user as returned by the list of user stores. This has been done to maintain the consistency of the results returned by the user stores for the search queries.
...
#### Groups claim
The representation of the group's claim in the ```UserInfo``` response has been changed. The group's claim is a multi-valued attribute, and it is represented accordingly as a JSON array in the ID token. But in earlier versions of the Identity Server, the value of the group claim in the UserInfo response is a single string of comma-separated values as shown below.

```"groups": "Internal/everyone,dev"```

With Identity Server 6.0.0, the UserInfo response has been updated to return a JSON array value for the group's claim, similar to that in the ID token.
...
From Identity Server 6.0.0 onwards, only client applications configured as Management Applications can access Server Management APIs.

Applications can be configured as management applications in the following methods:
- Check the **Management Application** option available on the application's edit page.
- Set the ```isManagementApp``` attribute to ```true``` using Application Management REST API for the required applications.

!!! Note
    This will affect only new applications. All applications that existed before the migration will still be able to access the Server's Management APIs.

## Database
This section covers the updates related to database configurations on Identity Server 6.0.0.

### H2 Database version upgrade
The H2 database engine version has been upgraded from ```1.4.199``` to ```2.1.210``` in Identity Server 6.0.0. As a result of this, Identity Server 6.0.0 will not be able to properly interact with the existing H2 databases of the older version.
...
If you have been using WSO2 Identity Server in a [Hazelcast cluster](https://is.docs.wso2.com/en/latest/administer/configuring-hazelcast/), you may have configured the logging type for Hazelcast as log4j ```(Log4j1)```. Log4j1 logging is no longer supported in the WSO2 Identity Server.

...
4.  Open the `sso.properties` file in the `<TOMCAT_HOME>/apache-tomcat-<version>/webapps/saml2-web-app-pickup-dispatch.com/WEB-INF/classes` folder and change the hostname in the SAML2 assertion consumer URL (`SAML2.AssertionConsumerURL` parameter) from `localhost.com` to `localhost` as shown below.

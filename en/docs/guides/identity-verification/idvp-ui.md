...
The metadata.json file contains the UI metadata that is required for rendering the dynamically generated content. Currently, this file is used when rendering the
...
- Configuration Properties section in the IDV Provider create/edit pages in the management console.
...
|------------------|--------------------------------------------------------------------------------------|--------------|
| identifier       | Renders a text input field that performs validations for identifiers by default.     | Console      |
| number           | Renders a numeric input field.                                                       | Console      |
| resource_name    | Renders a text input field that performs validations for resource names by default.  | Console      | 
| client_id        | Renders a text input field that performs validations for client ids by default.      | Console      |
| description      | Renders a text area field that performs validations for descriptions by default.     | Console      |
| email            | Renders a text input field that performs validations for email addresses by default. | Console      |
| url              | Renders a text input field that performs validations for URLs by default.            | Console      |     
| password         | Renders a password input field with a show/hide button.                              | Console      |
| text_area        | Renders a text area field.                                                           | Console      |
| checkbox         | Renders a checkbox.                                                                  | Console      |
| toggle           | Renders a toggle button.                                                             | Console      |
| dropdown         | Renders a dropdown.                                                                  | Console      |
| default          | Renders a text input field.                                                          | Console      |
...
| type            | Input field type out of one of the supported input field types specified above. The “default” input type is used if the type is not specified.                              | “password”                       | 
...
Apart from the above-mentioned attributes, the following attributes are supported by every text and numeric input field type.
...
Since the relevant fields of the template are overwritten by the values provided by the UI, the default values provided through the metadata.json will take precedence over the default values provided through the template for config properties.
Example
...
### Copy the created JSON files into the Identity Server

Create an empty directory in the following path using the name of the particular IDV provider ` <IS_HOME>/repository/resources/identity/extensions/identity-verification-providers` 
and copy the created info, metadata, and template JSON files into the directory. Restart the server to apply the changes.

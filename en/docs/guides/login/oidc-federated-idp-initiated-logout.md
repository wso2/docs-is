...
To try this out, let's configure two WSO2 Identity Servers as the primary identity server (WSO2 IS 1) and the federated identity provider (WSO2 IS 2).

Let's configure two web applications, **Pickup-Dispatch** and **Pickup-Manager** as **Application1** and **Application2** respectively.

1. Register the primary IS (WSO2 IS 1) as a service provider in WSO2 IS 2.
2. Register WSO2 IS 2 as an identity provider in the primary IS (WSO2 IS 1).
3. Configure Pickup Dispatch as a service provider in the Primary IS.
4. Configure Pickup Manager as a service provider in WSO2 IS 2.

!!! note
    - Note that two servers must run on ports, 9443 (Primary IS) and 9444 (Secondary IS).
    - Since there can be issues with cookies when the same hostname is configured for both WSO2 identity servers 
    (primary and federated), you need to configure different hostnames for both servers. Follow [Change hostname of the WSO2 IS]({{base_path}}/deploy/change-the-hostname) guide to change the hostname of the Secondary IS. In this guide, the hostname of the Secondary IS is configured as `localhost.com`.
...
6. Expand the **OAuth2/OpenID Connect Configuration** section under the **Inbound Authentication Configuration** section and click **Configure**.
...
9. Add `https://localhost:9444/oauth2/token` as the **Identity Provider's Issuer Name**.
    ...
    !!! note
        **Identity Provider's Issuer Name** of the Identity Provider in **Primary IS** should be same as the **Identity Provider Entity ID** in the `Identity Providers > Resident > Inbound Authentication Configuration > OAuth2/OpenID Connect Configuration` in the **Secondary IS**.

10. Click **Register**.

### Register a service provider in WSO2 IS

1. Follow the steps in [Deploying the pickup-dispatch webapp](https://is.docs.wso2.com/en/latest/learn/deploying-the-sample-app/#deploying-the-pickup-dispatch-webapp) to download, deploy, and register the **Pickup-Dispatch** sample.
2. Once you have added the OIDC service provider, go to **Service Provider Configuration** and expand **Local & Outbound Authentication Configuration**.
3. Select **Federated Authentication** and from the drop-down list select **SecondaryIS**.
...
Follow the steps in [Deploying the pickup-manager webapp](https://is.docs.wso2.com/en/latest/learn/deploying-the-sample-app/#deploying-the-pickup-manager-webapp) to download, deploy, and register the Pickup-Manager sample.
...
After adding the configurations, restart WSO2 IS.

- Note that `iatValidityPeriod` should be in seconds.

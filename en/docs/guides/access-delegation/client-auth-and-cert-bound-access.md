...
1. Add the following configurations to the `<IS_HOME>/repository/conf/deployment.toml` file to deploy and configure the mutual TLS client authenticator artifacts.
    ...
    In the configurations above, add the relevant certificate header name as the `client_certificate_header` value (`x-wso2-mtls-cert` is given as a sample value).
...
To test whether the configurations work as desired, you can configure a service provider for a sample application. Follow the steps below to set up the sample application and configure the service provider. 

**Set up the sample application**

Follow the instructions in [Deploying the Sample App](../../learn/deploying-the-sample-app/#deploying-the-playground2-webapp) to set up the Playground2 web application.

...
!!! note "Note"
    The access token and the refresh token that received with the response are just randomly generated by the sample application. Those tokens are not valid tokens.

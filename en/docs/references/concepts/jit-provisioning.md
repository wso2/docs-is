...
!!! info
    Optionally, you can also set up JIT provisioning to provision the new users persisted in WSO2 IS to the external system as well using [outbound provisioning]({{base_path}}/provisioning-framework/#outbound-provisioning). 
...
!!! info "Related topics"
    - [Concept: Introduction to Provisioning]({{base_path}}/identity-provisioning-intro)
    - [Concept: Provisioning Framework]({{base_path}}/provisioning-framework)
    - [Guide: JIT Inbound Provisioning]({{base_path}}/guides/identity-lifecycles/jit-workflow)

...
-   **Federated IDP Initiated OIDC Back-Channel Logout**
    ...
    When it comes to OIDC identity federation in Identity Server, WSO2 IS acts as an RP to the federated identity provider. However, currently there is no mechanism to terminate the sessions and revoke tokens in WSO2 IS (RP), whenever there is a session update in the federated IDP (OP) side. 

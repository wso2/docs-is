...
After setting up MariaDB database. You can point the `WSO2_IDENTITY_DB` or 
`WSO2_SHARED_DB` or both to that MariaDB database by following below instructions.

!!! note
       
    WSO2 Identity Server currently does not provide scripts specific to MariaDB SQL.However
    you can use MySQL db scripts as an alternative.

### Changing the default datasource

1.  **Minimum Configurations for changing default datasource to MariaDB.**
 ...
 A Sample configuration is given below.
   ...
   4.  Download the MariaDB JDBC driver for the version you are using and

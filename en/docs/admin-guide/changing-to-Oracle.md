...
you need to follow in order to use an Oracle database for this purpose.
...
-   [Creating the database
    tables](#ChangingtoOracle-Creatingthedatabasetables)
...
data. After setting up the Oracle database to replace the default H2
database, either [change the default configurations of the
`          WSO2_CARBON_DB         `
datasource](#ChangingtoOracle-Changingthedefaultdatabase) , or
[configure a new datasource](#ChangingtoOracle-Configuringanewdatabase)
...
1.  Edit the default datasourceconfigurationinthe \<
...
1.  Add a new datasource with similar configurations as the
    [`           WSO2_CARBON_DB          `
    datasource](#ChangingtoOracle-Changingthedefaultdatabase) above to
    the \<
    `          PRODUCT_HOME>/repository/conf/datasources/         `

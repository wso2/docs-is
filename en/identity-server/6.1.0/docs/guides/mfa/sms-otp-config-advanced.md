...
Defines the maximum number of allowed resend attempts for a given session. Once this limit is exceeded, the authentication attempt will fail.
If this property is not configured, the resend attempt validation will be skipped, allowing unlimited resend attempts.

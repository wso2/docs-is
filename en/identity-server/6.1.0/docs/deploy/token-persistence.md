...
!!! note  
    This feature is available for **WSO2 Identity Server 6.1.0** as of **update level 264** and currently supports **H2** , **MySQL** and  **Oracle** databases.
...
!!! note "Tables required for this feature"

    - **If you want both non-persistent access tokens and non-persistent refresh tokens**, the following tables are sufficient:

    - `IDN_OAUTH2_REVOKED_TOKENS`
    - `IDN_SUBJECT_ENTITY_REVOKED_EVENT`

    - **However, if you are using long-lived JWT refresh tokens with token rotation upon refresh token grant**, persisting the refresh token is recommended. In that case, you will need the following tables in addition to the above:
...
1. Add following tables to the `IDENTITY_DB`.
    ...
    ??? Example "MYSQL-Cluster"
        ...
        /
        DROP TABLE IDN_OAUTH2_REFRESH_TOKEN CASCADE CONSTRAINTS;
        /
        DROP TABLE IDN_OAUTH2_REVOKED_TOKENS CASCADE CONSTRAINTS;
        /
        DROP TABLE IDN_SUBJECT_ENTITY_REVOKED_EVENT CASCADE CONSTRAINTS;
        /
        ...
        CREATE INDEX IDX_CONSUMER_USER_SCOPE_IDP ON IDN_OAUTH2_REFRESH_TOKEN
...
2. Add the following to the deployment.toml to enable the feature in WSO2 Identity Server. 
    ...
    !!! Tip
        If you have long living refresh token with refresh token rotation, then its recommended to store the refresh token. Following config can be used to enable to refresh token persistence while keeping the access token as non persistence. 
        ```toml
            [oauth.token_persistence]
            persist_access_token = false

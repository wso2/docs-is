...
!!! note "Restart via the command window"
    ...
    command. **NOTE:** This will run the server without displaying the debug logs. 
        ...
        If you wish to see the logs, use the `tail -f <IS_HOME>/repository/logs/wso2carbon.log` command. **NOTE** 
        the given path in the command, `<IS_HOME>/repository/logs/` is relative to the `bin` folder.

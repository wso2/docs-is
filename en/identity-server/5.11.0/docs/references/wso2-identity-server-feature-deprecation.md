...
Features/APIs in the server can be deprecated for the following reasons:
...
While deprecated features can still be available and support existing users, they will not be further enhanced nor recommended for use. Removal of deprecated capabilities will occur over a deprecation window, in which the feature will undergo several phases in the deprecation lifecycle. 
...
The feature deprecation matrix keeps track of the WSO2 Identity Server feature deprecation timelines with their respective EOL versions in accordance with the IAM feature maintenance policy. Users are advised to review if they make use of the feature in their current deployment and make plans to change their implementation. Please refer to the matrix to identify the target timelines of feature removal from the product and EOL for maintenance in order to plan your environment and projects accordingly.

The matrix includes the product release versions that each feature will be deprecated or retired from. The **FUTURE** tag is used to indicate deprecation/retired timelines that are not yet finalized, and **FUTURE** on the Deprecated column indicates that the feature is in **Planned** phase, and the deprecated timeline is not yet finalized.

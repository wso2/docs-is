...
Descriptions]({{base_path}}/references/troubleshoot/error-codes/) for more information on the standard error codes and descriptions of
...
!!! note
    Account lock errors are returned only when account locking is enabled on the server. Refer [User Account Locking]({{base_path}}/guides/account-configurations/account-security/account-locking/) document to enable account locking.

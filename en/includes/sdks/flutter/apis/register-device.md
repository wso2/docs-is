...
All operations are accessed through the `Asgardeo.instance` singleton.
...
try {
  ...
  final accountId = await Asgardeo.instance.pushAuth.registerDeviceWithToken(
...
!!! important
    The push notification provider must be configured in {{product_name}} before registering a device. See [Server-side Push Provider Configuration]({{base_path}}/sdks/flutter/guides/set-up-push-providers/#server-side-push-provider-configuration).
...
await Asgardeo.instance.pushAuth.registerDevice(
  qrCodeJson,
  deviceToken,
...
!!! tip "Tip: Selecting a provider based on platform"
    ...
    await Asgardeo.instance.pushAuth.registerDevice(qrCodeJson, deviceToken, provider);
    ```

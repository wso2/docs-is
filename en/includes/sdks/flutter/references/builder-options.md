`AsgardeoBuilder` configures the SDK before `build()` is called. All options have sensible defaults — only override what you need.
...
!!! note
    Calling `build()` more than once throws `AsgardeoAlreadyInitializedException`. `Asgardeo.reset()` exists for development and testing only — it disposes the singleton and allows re-initialization. Do not use `reset()` in production code.

For pluggable interfaces (`httpManager`, `storageManager`, `cryptoEngine`, `logger`, `deviceInfoProvider`), see [Custom Managers]({{base_path}}/sdks/flutter/references/custom-managers/).

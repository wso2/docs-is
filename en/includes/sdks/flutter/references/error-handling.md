...
|-----------|-------------|
| `AsgardeoNotInitializedException` | `Asgardeo.instance` is accessed before `build()` was called |
...
```dart
try {
  await Asgardeo.instance.pushAuth.registerDevice(qrCodeJson, deviceToken, FCMPushProvider());

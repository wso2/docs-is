...
<tr class="odd">
<td>event</td>
<td><p>Contains context information relevant to access token issue flow. Refer <a href="#event">event</a> section for details.</p></td>
</tr>
<tr class="even">
...
<tr>
<td>fields</td>
<td>An array of the top level field names that will be present on the token endpoint's response, based on the current token request, such as <code>access_token</code>, <code>scope</code>, <code>expires_in</code>, and, if applicable, <code>refresh_token</code> and <code>id_token</code>. Your external service can use this list to add new custom fields to the response without a name collision, or to remove the optional <code>refresh_token</code> or <code>id_token</code> fields from the response.</td>
</tr>
</tbody>
</table>
</td>
...
Your external service can also add custom top level fields to the token endpoint response, or suppress optional standard fields, such as <code>refresh_token</code> or <code>id_token</code>, from it. These operations use the <code>/response/fields/</code> path, which relates to the <code>event.response</code> property.
...
<tr class="even">
<td>errorMessage</td>
<td><p>Describes the cause of the error</p></td>
</tr>
<tr class="odd">

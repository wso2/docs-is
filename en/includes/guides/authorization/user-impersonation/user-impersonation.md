...
User impersonation allows an authorized user to act on behalf of another user. This is useful for administrators or support personnel who need to perform tasks without requiring the credentials of the end user.

This guide explains how to enable user impersonation in {{product_name}}.

## Prepare the Impersonating Application

By registering the User Impersonation API resource, you authorize the application to be used for impersonation.

### Register User Impersonation API resource

By registering the user imperonation API resource, we will be authorizing the application to be used for impersonation.
...
![Authorize impersonation API]({{base_path}}/assets/img/guides/authorization/impersonation/api-authorization-impersonation.png)
...
Since the user is pre-approved to impersonate another user, we can bypass the consent prompts during both login and logout processes. This is disabled by default. If you have already enabled this feature and want to disable it:

1. Switch to the **Advanced** tab of the application.
2. Select **Skip login consent** and **Skip logout consent**.
3. Click **Update** to save the changes.

## Prepare the Impersonating User

Impersonating users must be granted the appropriate permissions via the application role.

1. Go to the Roles page.
2. Select the **Impersonator** role with your Business Application as the audience.
    (This is the role you have created while regsitering the API resource.)
3. Under the Users tab, assign the appropriate users.
4. Ensure the users also have administrator privileges as required.

## Ways to Initiate User Impersonation

User impersonation can be initiated in two ways:

   1. [Via the Console](#1-initiating-user-impersonation-via-console)
   2. [Via a Business Application](#2-initiating-user-impersonation-via-a-business-application)

### User Session Impersonation

Once you initiate impersonation from any of above two methods, the user session becomes an impersonated session. This means the impersonating actor can get single sign on (SSO) into other [impersonation authorize applications](#register-user-impersonation-api-resource) if the impersonating actor has the [necessary permission to perform impersonation](#prepare-the-impersonating-user) using the application.

## 1. Initiating User Impersonation via Console
...
4. Under the Users tab, assign the impersonation role to the user.
5. Grant administrator privileges to the selected user. [Follow this guide]({{base_path}}//guides/users/manage-administrators/#assign-admin-privileges-to-users).
...
3. Scroll to the bottom and click on **Impersonate User** button.

    ![User Impersonate Button]({{base_path}}/assets/img/guides/authorization/impersonation/user-impersonate-button.png){: width="600" style="display: block; margin: 0; border: 0.3px solid lightgrey;"}

4. You will be logged into the My Account application as the impersonated user.

After impersonation is initiated via the Console, you can now SSO into any Business Application that is authorized for impersonation and where the impersonating user has the required application role with impersonation permissions.

## 2. Initiating User Impersonation via a Business Application

### Step 1: Authorize the Application for User Impersonation

As described in section [Prepare the Impersonating Application](#prepare-the-impersonating-applicationn) register the user impersonation API resource for the application and create the Application Role.
...
### Step 3: Assign the Impersonation Role to Users

Assign the **Impersonator** Role to users as described in section [Prepare the Impersonating Application](#prepare-the-impersonating-application).

# Start impersonation via business application

This guide will take you through the steps you need to follow when impersonation initiation has to performed via a business application instead of **My Account**.

## Step 1: Configure the application to perform impersonation

Create a Business Application and update the application with the instructions in [this section]({{base_path}}/guides/authorization/user-impersonation/via-console/#step-3-configure-business-application-to-perform-impersonation) to configure the application to perform impersonation.
...
Assign the **Impersonator** role to users as described in section [Prepare the Impersonating Actor]({{base_path}}/guides/authorization/user-impersonation/via-console/#step-1-prepare-the-impersonating-actor).
...
A subject token refers to a JWT token containing details about both the impersonated user and the impersonator. To get a subject token, the client needs to start an authorization request using the following query parameters.
...
<table>
    ...
    <tr>
        <td>redirect_uri</td>
        <td>The address to where the response redirected to at the end of the process.</td>
    </tr>
    <tr>
        <td>scope</td>
...
After successful authorization, the response will look like the following:
...
The decoded subject token may look as follows:
...
The subject token also includes the `may_act` property. This property indicates that the user with the ID in the `may_act.sub` field has permission to impersonate the user identified by the ID in the `sub` field.

#### Get an impersonated access token

Once a subject token received, exchange it for an access token which represents the permissions of the impersonated user.
...
The `act.sub` property holds the user ID of the impersonator.

In the example given, the `sub` property holds the value of `32bc4697-ed0f-4546-8387-dcd6403e7caa`, which the user ID of the impersonated user. The `act.sub` property holds the value of `2d931c9d-876e-46c0-9aba-f34501879dfc`, which the user ID of the impersonator.

...
To configure OpenID Connect hybrid flow for authentication, please follow the following steps:

## Prerequisites

To get started, you need to have an application registered in {{ product_name }}. If you don't already have one, register a [OIDC application]({{base_path}}/guides/applications/register-oidc-web-app/#register-app).

1. On the {{ product_name }} Console, go to **Applications**.
2. Select the configured Application.
3. Go to the **protocol** section.
4. Enable authorization code grant. Now the Hybrid flow configuration will appear in the protocol section.

![Enbale auth_code flow]({{base_path}}/assets/img/guides/authentication/enable_auth_code_flow.png){: width="800" style="display: block; margin: 0; border: 0.3px solid lightgrey;"}

5. Select the `Enable Hybrid Flow` checkbox and choose the desired hybrid flow response type from the options provided.

![Enbale hybrid flow]({{base_path}}/assets/img/guides/authentication/enable_hybrid_flow.png){: width="800" style="display: block; margin: 0; border: 0.3px solid lightgrey;"}

!!! warning

    It is not recommended to use `code token` and `code id_token token` response types, as obtaining the access token directly 
    from the authorization endpoint without client authentication introduces potential security vulnerabilities, including the risk of account takeover attacks.

...
2.  Replace your keystore file path, keystore password, trust store file path and trust store password (you can use the keystore and
    ...
    !!! warning "Port configuration behavior"
        When you configure the custom x509 transport with a specific port (for example, `port="8443"`), this port will be used in all redirect URLs generated by {{ product_name }}, including the management console URL, My Account URL, and authentication endpoints. This configuration overrides the `proxyPort` setting defined in the `[transport.https.properties]` section.

        If you are deploying {{ product_name }} behind a proxy or load balancer with a different external port, ensure that:

        - The port specified in the `[custom_transport.x509.properties]` section matches your external port, or
        - The firewall allows traffic on the configured x509 transport port

        Otherwise, users may encounter connection issues when the system redirects to URLs using the x509 transport port.

...
Again you don’t need to attend the token validation by yourself, following the security best practices, the WSO2 Identity Platform SDK enables ID token validation by default, using the JWKS (JSON Web Key Set) endpoint of {{product_name}} to validate the signature. The SDK constructs the JWKS and other necessary endpoints based on the baseUrl, so providing the baseUrl is typically sufficient. To give you an idea about what happens behind the scenes, the WSO2 Identity Platform React SDK retrieves the public key of your {{product_name}} organization through the JWKS endpoint and uses it to verify whether the token has been signed by the private key of the {{product_name}} . If required, you can modify the JWKS endpoint and other parameters. 
...
The WSO2 Identity Platform React SDK also validates other attributes such as the issuer and expiration period, with a default clock tolerance of 60 seconds.
...
There are several storage mechanisms available in the browser, such as Session Storage, Local Storage, Web Workers, and Browser Memory. By default, the WSO2 Identity Platform SDK stores session information, including the access token, in Session Storage. However, this can be configured as per your requirements. Here is an example using WSO2 Identity Platform React SDK.

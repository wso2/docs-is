...
<table>
     ...
     <tr>
       <td><code>tenantDomain</code> or <code>organizationId</code></td>
       <td>The tenant domain of the tenant user or the organization ID of the organization user.</td>
     </tr>
     <tr>
       <td><code>organizationName</code> (Optional)</td>
       <td>Organization name of the sub organization user.</td>
     </tr>
     <tr>
...
### Generate push authenticator app unique identifier
...
### Generate the signature for registration verification
...
3. Concatenate the challenge and the device token using `.` as the separator. (Example: `challenge.deviceToken`)
4. Sign the concatenated value using the **private key** generated during the registration process. Use **PKCS#1 v1.5 padding** as the padding scheme and apply **SHA-256** before encryption.
5. Encode the signature in **base64** format.

### Invoke the registration API
...
<table>
     ...
     <tr>
       ...
       <td>The unique identifier token generated by the notification provider service for the app instance.</td>
     </tr>
     <tr>
       <td><code>publicKey</code></td>
       <td>Base64 encoded, header and footer less PEM value of the public key generated by the registering device.</td>
     </tr>
     <tr>
       <td><code>signature</code></td>
       <td>Base64 encoded, hashed and encrypted string value signed by the private key from the registering device</td>
     </tr>
</table>
...
If response contains the **tenant domain**, the registration API should be invoked with the **tenanted path** as shown below.

    https://localhost:9443/t/{tenant-domain}/api/users/v1/me/push/devices/

If response contains the **organization ID**, the registration API should be invoked with the **organization path**.

...
An ID token is a signed JWT issued by {{product_name}} as part of an OpenID Connect authentication flow. The client application uses it to verify who authenticated, how, and when — ID tokens must not be sent to resource servers as authorization credentials.

...
DPoP (Demonstration of Proof-of-Possession) token binding method binds the access token to the private key of the client. A DPoP proof generated by the client using the private key, is included in the `DPoP` header of each request. This proof is required to demonstrate proof of possession, ensuring that the token can only be used by the client that holds the corresponding private key. This method is supported with all grant types.
{% endif %}

...
- An M2M token obtained during a billing cycle counts toward that month's M2M token quota.
...
- An M2M token that is valid for a longer time and overlaps two or more billing cycles counts as a single token in its issue month, unless renewed using either token refresh or token exchange mechanism.
...
- Every organization present under the root organization during a billing cycle counts toward that month's total.
...
- An organization continues to count in each billing cycle that it remains, and no longer counts after deletion.
...
- Each enterprise identity provider connection configured within the organization or its sub-organizations during a billing cycle counts toward that month's Enterprise SSO connection total.

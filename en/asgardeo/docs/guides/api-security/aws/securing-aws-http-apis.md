...
You will need a Client ID and Client Secret from an application that is authorized to invoke the management APIs. You can use an existing application or create a new application in the WSO2 Identity Platform console with the **Client Credentials** grant enabled. Make sure the application has the scope `internal_application_mgt_create` (this scope is required to create a new application via the API).

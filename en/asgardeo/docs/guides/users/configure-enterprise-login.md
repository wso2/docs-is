# Configure enterprise login for the {{product_name}} Console
...
This guide walks you through configuring enterprise login for the {{product_name}} Console by setting up an enterprise connection and mapping external IdP groups to {{product_name}} Console roles.

## Prerequisites

- You need to have an {{product_name}} organization with administrator access.
...
First, register your external enterprise IdP as a connection in {{product_name}}.
...
2. After creating the connection, configure the following settings:
    ...
    <table>
        ...
        <tr>
            ...
            <td>Enable JIT provisioning for the connection. This ensures that user accounts are automatically created in {{product_name}} when users sign in through the enterprise IdP for the first time.</td>
        </tr>
        <tr>
            <td><b>Groups claim mapping</b></td>
            <td>Add a claim mapping from the external IdP's groups claim to the local groups claim. This ensures that group information from the external IdP is correctly mapped to {{product_name}}.</td>
        </tr>
        <tr>
...
1. On the {{product_name}} Console, go to **Console Settings** > **Enterprise Login**.
...
3. Under **Group-Role Mappings**, click **Add Mapping** to map IdP groups to {{product_name}} Console roles.
...
4. For each mapping, configure the following:
    ...
    <table>
        ...
        <tr>
            ...
            <td>Select the {{product_name}} Console role to assign. For example, select <b>Administrator</b> to grant full administrative access.</td>
        </tr>
        <tr>
    ...
    !!! info
        Add additional mappings for any other console roles you want to assign to your enterprise users. Each mapping links an external IdP group to a specific {{product_name}} Console role.
...
After you save the configuration, users who belong to the mapped IdP groups can sign in to the {{product_name}} Console using your enterprise IdP.

## Sign in with enterprise login

After you configure enterprise login, users can sign in to the {{product_name}} Console through the enterprise IdP from the standard {{product_name}} login page.
...
1. Go to the connection settings of your enterprise IdP in {{product_name}}.
...
3. Share the following URL with your enterprise users for direct access:
    ...
    <table>
        ...
        <tr>
            ...
            <td>Your {{product_name}} organization name.</td>
        </tr>
        <tr>
    ...
    Users who navigate to this URL go directly to the enterprise IdP's sign-in page and are redirected to the {{product_name}} Console after successful authentication.
...
| Groups are not mapped correctly | Confirm that the group names configured in {{product_name}} exactly match the group names sent by your external IdP in its authentication response. Group names are case-sensitive. |
| JIT provisioning is not creating user accounts | Verify that JIT provisioning is enabled on the enterprise connection. Check the {{product_name}} provisioning logs for error details. |
...
- Monitor sign-in activity in the {{product_name}} Console audit logs to detect any authentication issues.
...
- Share the direct access URL with your enterprise administrators so they can access the {{product_name}} Console directly through your enterprise IdP.
